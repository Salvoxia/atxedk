...
{foreach from=$involved key="key" item="i"}
		<tr class="{cycle name="ccl"}">
			...
			<td><a href="{$i.pilotURL}"><font color="#fff"><b>{$i.pilotName}</b></font></a></td>
		</tr>
		<tr class="{cycle name="ccl"}">
			<td><a href="{$i.corpURL}">{$i.corpName}</a> [ <i>{if $i.alliURL}<a href="{$i.alliURL}">{$i.alliName}</a>{else}{$i.alliName}{/if}</i> ]</td>
		</tr>
		<tr class="{cycle name="ccl"}">
		...
		<tr class="{cycle name="ccl"}">
			<td>Damage done: <font color="#bcbcbc">{$i.damageDone} {if $victimDamageTaken > 0}</font>({($i.damageDone/$victimDamageTaken*100)|string_format:"%.2f"}%){/if}</td>
		</tr>
{/foreach}
